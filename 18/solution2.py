...
def parse(s) -> list[int]:
    """
    Parse the 'Snail fish number' string to a list of its components
    :param s: the 'Snail fish number'
...
def explode(s) -> tuple:
    ...
    for i in range(len(items)):
        ...
        if item == '[':
            ...
            # we are measuring 'depth' only so ignoring other items
            pass
    # nothing to explode
    return s, False
    ...
    for i in range(len(items)):
        ...
        if isinstance(item, int):
            ...
            if value < 10:
                ...
                continue
            # found the index of the (int) item to split
            # remove the number
            items.pop(i)
...
def get_magnitude(s) -> tuple:
    ...
    for i in range(len(items)-2):
        if items[i] == '[' and isinstance(items[i + 1], int) and \
                items[i + 2] == ',' and isinstance(items[i + 3], int) and items[i + 4] == ']':
